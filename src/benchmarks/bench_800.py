...
N_TEST_VIEWS = 3
TARGET_PSNR  = 25.0
CONFIGS      = [(200, 4096), (800, 4096)]   # (resolution, batch_rays)
...
print("Loading Lego at native resolution...", flush=True)
...
print(f"  {len(train_full)} train, {len(test_full)} test, native {native_W}", flush=True)

summary = []
for res, batch in CONFIGS:
    cov = batch * N_ITERATIONS / (len(train_full) * res * res)
    print(f"\n=== {res}x{res}  batch={batch}  (coverage {cov:.2f}x per image) ===", flush=True)
    ...
    hit = next(((it,w) for (it,w,p,s) in curve if p>=TARGET_PSNR), (None,None))
    summary.append((res, batch, cov, per_iter_ms, vram, best_p, best_s, total, hit[0], hit[1]))
    del train_images, test_images, model, encoding, opt
    torch.cuda.empty_cache()

print("\n\n========== SUMMARY ==========")
print(f"{'res':>5} {'batch':>6} {'cov':>6} {'ms/it':>7} {'VRAM_MB':>9} {'bestPSNR':>9} {'bestSSIM':>9} {'run_s':>8} {'it->25dB':>9}")
for (res,batch,cov,ms,vram,bp,bs,tot,hi,hw) in summary:
    hit_s = str(hi) if hi else "not hit"
    print(f"{res:>5} {batch:>6} {cov:>5.2f}x {ms:>7.2f} {vram:>9.1f} {bp:>9.2f} {bs:>9.4f} {tot:>8.1f} {hit_s:>9}")
print("\nReference from first benchmark (20k iters, batch 1024):")
print("  100x100: best PSNR 23.51 / SSIM 0.890   |   800x800: best PSNR 21.15 / SSIM 0.775")
